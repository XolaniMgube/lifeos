      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 pb-28 md:pb-10">
        ...
        <section className="mb-10">
          ...
          <Link
            ...
            className="block bg-bg-surface border border-line rounded-xl p-5 sm:p-6 hover:border-accent/40 transition-colors group grain"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-widest text-ink-tertiary mb-2">Day {nextDay}</p>
                <h3 className="display-font text-2xl sm:text-3xl text-ink-primary mb-1">{nextDayDef.name}</h3>
        ...
        {/* Goals */}
        <section className="mb-10">
          <SectionHeader title="Goals in play" sub="Outcomes linked to work" />
          {activeGoals.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {activeGoals.map((goal) => {
                const linked = tasks.filter((t) => t.goalId === goal.id && t.status !== 'cancelled');
                ...
                return (
                  <Link
                    key={goal.id}
                    href="/goals"
                    className="bg-bg-surface border border-line rounded-lg p-4 hover:border-accent/40 transition-colors"
                  >
                    <p className="text-[10px] uppercase tracking-widest text-ink-tertiary mb-2">
                      {goal.horizon}{goal.area ? ` · ${goal.area}` : ''}
                    </p>
                    <h3 className="text-sm font-medium text-ink-primary truncate mb-3">{goal.title}</h3>
                    <div className="h-1.5 rounded-full bg-bg-inset overflow-hidden mb-2">
                      <div className="h-full rounded-full bg-accent" style={{ width: `${percent}%` }} />
                    </div>
                    <p className="text-xs text-ink-tertiary">{done} of {linked.length} tasks done</p>
                  </Link>
                );
              })}
            </div>
            ...
            <Link
              href="/goals"
              className="block bg-bg-surface border border-dashed border-line rounded-lg p-5 text-sm text-ink-secondary hover:border-accent/40 transition-colors"
            >
              Add the first goal, then attach tasks to it.
            </Link>
          )}
        </section>
...
function StatCard({ label, value, sub, accent }: { label: string; value: string; sub: string; accent?: boolean }) {
  return (
    <div className="bg-bg-surface border border-line rounded-lg sm:rounded-xl p-4 sm:p-5">
